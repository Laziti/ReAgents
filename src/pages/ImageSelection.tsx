...
const ImageSelection = () => {
  ...
  const handleNext = () => {
    ...
    // Pass image data to the next page (only File objects, not blob URLs)
      ...
      <div className="flex-1 flex flex-col overflow-hidden md:overflow-hidden min-h-0">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 pb-24 md:pb-6 min-h-0" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
          <div className="max-w-4xl mx-auto">
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-2xl sm:text-3xl font-bold text-[var(--portal-text)] mb-2">Select Images</h1>
            ...
            <div className="bg-[var(--portal-card-bg)] rounded-lg shadow-lg border border-[var(--portal-border)] p-4 sm:p-6">
              <div className="space-y-6 sm:space-y-8">
                ...
                <div>
                  ...
                  <div className="mt-1 flex justify-center px-4 sm:px-6 pt-5 pb-6 border-2 border-dashed border-[var(--portal-border)] rounded-lg bg-[var(--portal-bg)]/40">
                      ...
                      {mainImagePreview ? (
                        <div className="relative">
                          <img 
                            ...
                            className="mx-auto h-48 sm:h-64 object-cover rounded-md shadow-sm w-full max-w-md" 
                          />
                          <button
                            type="button"
                        ...
                        <>
                          <svg
                            className="mx-auto h-16 sm:h-20 w-16 sm:w-20 text-[var(--portal-text-secondary)]"
                            stroke="currentColor"
                            fill="none"
                            viewBox="0 0 48 48"
                            aria-hidden="true"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 mt-2">
                    {additionalImagePreviews.map((preview, index) => (
                      <div key={index} className="relative h-32 sm:h-40 border rounded-lg overflow-hidden shadow-sm bg-[var(--portal-bg)]/40">
                        <img 
                          src={preview} 
                    ...
                    {additionalImages.length < 5 && (
                      <label
                        ...
                        className="h-32 sm:h-40 border-2 border-dashed border-[var(--portal-border)] rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-[var(--portal-bg-hover)] transition-colors bg-[var(--portal-bg)]/40"
                      >
                        <Plus className="h-6 sm:h-8 w-6 sm:w-8 text-gold-500" />
                        <span className="text-xs sm:text-sm text-[var(--portal-text-secondary)] mt-2 font-medium">Add Image</span>
                        <input
